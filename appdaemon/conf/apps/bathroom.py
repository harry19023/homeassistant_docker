...
class Bathroom(hass.Hass):
  ...
  def lights_off(self, entity, attribute, old, new, kwargs):
    door = self.get_state('binary_sensor.door_window_sensor_bathroom_door')
    if door == 'on':
      self.globals.debug('Turning off bathroom lights', self.debug)
      self.turn_off('light.bathroom')
    else:
      self.globals.debug("Didn't turn off bathroom lights. door: " + str(door), self.debug)
