...
class Harmony(hass.Hass):
  ...
  def initialize(self):
    ...
    self.listen_state(self.harmony_change, 'remote.harmonyhub', attribute='current_activity')

    self.log('Successfully initialized Harmony!' , level='INFO')


  def everything_off(self, entity, attribute, old, new, kwargs):
    self.call_service('remote/turn_off', entity_id='remote.harmonyhub')
  ...
  def stream_on(self, entity, attribute, old, new, kwargs):
    if old is not None:
      ...
      if ent in self.stream_devices:
        current_activity = self.get_state('remote.harmonyhub', attribute='current_activity')
        if current_activity != self.ent_to_activity[ent]:
          self.call_service('remote/turn_on', entity_id='remote.harmonyhub', activity=self.ent_to_activity[ent])
